...
void ae3d::SpriteRendererComponent::Impl::CreateVertexBuffer()
{
    ...
    // FIXME: evaluate perf and consider other containers.
    ...
    {
        const float width = static_cast<float>(sprites[i].texture->GetWidth());
        const float height = static_cast<float>(sprites[i].texture->GetHeight());
...
void ae3d::SpriteRendererComponent::Impl::Render()
{
    ...
    {
        BuildRenderQueue();
        ae3d::System::Print("building rendre");
    }
